...
%        Sim. time [0.5$s$, 5$s$] 
      ...
      \begin{mycell}{3.5cm} 94.94\%\\Latency\\Biology time \end{mycell} \\% recognition
      %
...
\label{subsec:model}
As we are proposing spike based data-sets or a methodology to produce them, it's desirable that the users specify whether they added any further processing either to images or spikes. This is important, for example, if we want to use a modified set to test noisy inputs~[\cite{best-practice-nn-img}].
