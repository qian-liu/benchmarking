...
\label{sec:Related}
In conventional computer vision, there are a few datasets playing import roles in different time and aiming at various targets.
\subsection{MNIST~\citep{lecun_gradient-based_1998}}
...
\subsection{ImageNet~\citep{deng_imagenet:_2009}}
...
Synsets are meaningful concepts described with a few words or phrases, and they are organised in a hierarchy as in WordNet.
...
The dataset is a well-recognised benchmark test for the deep learning community, and many attempts have been made to improve the performance, for example~\citep{krizhevsky2012imagenet}.

\subsection{Microsoft COCO~\citep{lin_microsoft_2014}}
...
Two early benchmarks, the KTH ~\citep{schuldt2004recognizing} and Weizmann~\citep{blank2005actions}, have been used extensively in the past decade. 
...
The YouTube Action Dataset~\citep{liu_recognizing_2009} targets recognising realistic actions from videos ``in the wild''.
...
%There are attempts~\citep{la2008response}~\citep{burkitt2006review} to estimate the output firing rate of the LIF neurons (Equation~\ref{equ:lif}) under certain conditions. 
...
%The response function of the LIF neuron with Poisson input spike trains is given by the Siegert function~\citep{siegert1951first}, 
%%\begin{equation}
