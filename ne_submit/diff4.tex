...
\DIFdelbegin \DIFdel{In the case of visual processing, the central visual system consists of several cortical areas which, according to anatomical experiments, are placed in a hierarchical structure ~\mbox{\citep{felleman1991distributed}}.
...
Biological studies have revealed that the information is unfolded along the ventral stream to the IT (Inferior Temporal) cortex~\mbox{\citep{dicarlo2012does}}.
...
\DIFaddbegin \DIFadd{Achieving the brain's energy efficiency motivates the development of neuromorphic hardware, since the brain consumes only about 20~W of power~\mbox{\citep{drubach2000brain}}.
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. in 200~ms in monkeys~\mbox{\citep{fabre1998rapid}} and every image is presented less than 100~ms~\mbox{\citep{keysers2001speed}}.
Such rapid and highly accurate recognition is the targets of modelling spike-based visual recognition.}
...
\DIFaddbegin \DIFadd{Inspired by biological studies of the visual ventral pathway, SNN models have successfully been adapted to visual recognition.} \cite{riesenhuber1999hierarchical} proposed a quantitative modelling framework for object recognition with position-, scale- and view-invariance.
...
\DIFdelbegin \DIFdel{They employed simple integrate-and-fire neurons with rank order coding (ROC) where the earliest pre-synaptic spikes have the strongest impact on the post-synaptic potentials.}
...
\DIFdelbegin \DIFdel{Using one spike per neuron,~\mbox{\cite{delorme2001face}} reported 100\% and 97.5\% accuracies on the face identification task over changing  contrast and luminance training (40 individuals $\times$ 8 images) and testing data (40 individuals $\times$ 2 images) respectively.
In terms of vision recognition on neuromorphic hardware, TrueNorth~\mbox{\citep{merolla2014million}} has been used to detect and recognise five classes of object in the video clips of the Neovision2 Tower dataset.
\mbox{\cite{Qiao2015re}} presented an analogue on-line learning neuromorphic classification system which responds selectively to images of cars and motorbikes selected from the Caltech 101 dataset.
Generic training algorithms for pattern recognition~\mbox{\citep{schmuker2014neuromorphic}} and regression/classification tasks~\mbox{\citep{thakur2016low}} have been implemented on neuromorphic platforms.
The Convolutional Neural Network (CNN), also known as the \mbox{\textit{ConvNet}}, developed by~\mbox{\cite{lecun1998gradient}}, is a widely-used model based on a cortex-like framework.}
\DIFaddbegin \DIFadd{Using one spike per neuron, similar to the first spiking wave in biology,~~\mbox{\cite{delorme2001face}} reported 100\% and 97.5\% accuracies on the face identification task over
...
\DIFaddbegin \DIFadd{~\mbox{\cite{hubel1962receptive}} first discovered the model of orientation selectivity (simple cells) and pooling mechanism (complex cells) in the primary cortex in cats, which lay the foundation of the Convolutional Neural Network (CNN)~~\mbox{\citep{lecun1998gradient}}.}
...
It was capable of recognising three moving postures with a CA of about 99.48\% and \DIFaddbegin \DIFadd{ classifying hand-written digits with} 88.14\% \DIFaddbegin \DIFadd{ accuracy} on the MNIST-DVS dataset (see \DIFdelbegin \DIFdel{Chapter} \DIFaddbegin \DIFadd{ Section}~\ref{sec:data}).
...
\DIFaddbegin \DIFadd{Spike-Timing-Dependent Plasticity (STDP) as  a learning mechanism based on biological observations has been applied to vision tasks.
\mbox{\cite{bichler2012extraction}} demonstrated an unsupervised STDP learning model to classify car trajectories captured with a DVS retina. 
A similar model was tested on a Poissonian spike presentation of the MNIST dataset achieving a performance of 95.0\%~\mbox{\citep{diehl2015unsupervised}}.
Theoretical analysis~\mbox{\citep{nessler2013bayesian}} showed that unsupervised STDP was able to approximate a stochastic version of Expectation Maximisation, a powerful learning algorithm in machine learning.
A computer simulation achieved a 93.3\% CA on MNIST and had the potential to be implemented using memristors~\mbox{\citep{bill2014compound}}.}

Deep Neural Networks (DNNs)\DIFdelbegin \DIFdel{, together with deep learning, are the most exciting research field in computer vision} \DIFaddbegin \DIFadd{have exceeded human-level performance on image classification tasks~\mbox{\citep{he2015delving}}}, but mainstream DNN research is focussed on continuous rather than spiking neural networks.
...
An event-driven Contrastive Divergence (CD) training algorithm for Restricted Boltzmann Machines (RBMs) was proposed for Deep Belief Networks (DBNs) using LIF neurons with \DIFdelbegin \DIFdel{Spike-Timing-Dependent Plasticity}STDP synapses and verified on MNIST with a CA of 91.9\%~\citep{neftci2013event}.

\DIFdelbegin \DIFdel{STDP as a biological learning process has been applied to vision tasks.
\mbox{\cite{bichler2012extraction}} demonstrated an unsupervised STDP learning model to classify car trajectories captured with a DVS retina. 
A similar model was tested on a Poissonian spike presentation of the MNIST dataset achieving a performance of 95.0\%~\mbox{\citep{diehl2015unsupervised}}.
Theoretical analysis~\mbox{\citep{nessler2013bayesian}} showed that unsupervised STDP was able to approximate a stochastic version of Expectation Maximisation, a powerful learning algorithm in machine learning.
A computer simulation achieved a 93.3\% CA on MNIST and had the potential to be implemented using memristors~\mbox{\citep{bill2014compound}}. }
...
\DIFdelbegin \DIFdel{However, in the field of conventional non-spiking computer vision, there are a number of datasets playing important roles at different times and with various objectives.
The MNIST~\mbox{\citep{lecun1998gradient}} dataset is a subset of the NIST hand-written digits dataset; due to its straightforward target of classifying real-world images, the plain format of the binary data and simple patterns, MNIST has been one of the most popular datasets in computer vision for over 20 years.
ImageNet~\mbox{\citep{deng2009imagenet}} was put forward to provide researchers with a large-scale image database that currently contains 14,197,122 images;
this dataset is a well-recognised benchmark test for the deep learning community, and many attempts have been made to improve the performance of machine learning algorithms on this dataset, as was done, for example, by \mbox{\citet{krizhevsky2012imagenet}}.
...
Two early benchmarks, the KTH ~\mbox{\citep{schuldt2004recognizing}} and Weizmann~\mbox{\citep{blank2005actions}} datasets, have been used extensively in the past decade. 
These videos were produced with scripted behaviours in a controlled environment (``in the lab'').
The YouTube Action Dataset~\mbox{\citep{liu2009recognizing}} targets recognising realistic actions from videos ``in the wild'';
...
%here the main challenge is in the massive variations due to the moving camera, background clutter, viewing angles, variable illumination and so on.
In consequence, a new set of spike-based vision datasets is now needed to quantitatively measure progress within \DIFdelbegin \DIFdel{this} \DIFaddbegin \DIFadd{the} rapidly advancing field \DIFaddbegin \DIFadd{of spike-based visual recognition} and to provide resources to support fair competition between researchers.

\DIFdelbegin \DIFdel{Apart from using spikes instead of the frame-based data used in conventional computer vision, new concerns arise when evaluating neuromorphic vision in addition to recognition accuracy.
...
\DIFaddbegin \DIFadd{Apart from using spikes instead of the frame-based data used in conventional computer vision, new concerns arise when evaluating neuromorphic vision, such as latency and energy consumption, in addition to recognition accuracy.
...
\DIFdelbegin \DIFdel{Running various SNN benchmarks demonstrates the capability of a hardware platform to support model-specific configurations such as network size, topology, neural and synaptic models, learning algorithms and so on.}
...
\begin{itemize}
	...
	\DIFdelbegin \DIFdel{With the growing understanding of biological vision, new methodologies and algorithms that can convert these conventional datasets into spikes in more biologically-accurate ways are welcome.}
	\DIFaddbegin \DIFadd{More biologically-accurate and better information preserving schemes are welcome.}
...
%	Other spike representation schemes may do a better job of preserving information from the original stimuli.
	...
	In order to reveal the advantages of neuromorphic engineering\DIFaddbegin \DIFadd{: highly accurate, rapid, energy-efficient recognition}, we need not only a spike-based dataset but also an appropriate evaluation methodology.
...
\DIFaddbegin \DIFadd{Due to its straightforward target of classifying real-world images, the plain format of the binary data and simple patterns, MNIST has been one of the most popular datasets in computer vision for over 20 years.
...
Rank-ordered \DIFdelbegin \DIFdel{encoding} \DIFaddbegin \DIFadd{coding} can be performed using an algorithm known as the
...
\DIFdelbegin \DIFdel{describes}\DIFaddbegin \DIFadd{shows} the parameters \DIFaddbegin \DIFadd{ (described in~\mbox{\citealp{sen2009evaluating}})} used to compute the convolution kernels at each 
scale $w$.
...
\label{sec:eval}
As a result of the spike-based processing used in SNN models, new concerns \DIFaddbegin \DIFadd{ on the latency and energy cost} \DIFdelbegin \DIFdel{(outlined below)} arise over performance assessment.
...
The proposed evaluation metrics \DIFdelbegin \DIFdel{(see Table 5)} consist of the \textbf{feasibility}, \textbf{classification accuracy}, \textbf{simulation time} and \textbf{energy use}.
...
\DIFdelbegin \DIFdel{Although spike-based algorithms have not surpassed their non-spiking counterparts in terms of recognition accuracy, they have shown great performance in response time and energy efficiency.}
