...
\DIFaddbegin \DIFadd{Achieving the brain's energy efficiency motivates the development of neuromorphic hardware, since the brain only consumes about 20~W of power~\mbox{\citep{drubach2000brain}}.
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. 200~ms in monkeys~\mbox{\citep{fabre1998rapid}} and the images are presented sequentially in spikes less than 100~ms~\mbox{\citep{keysers2001speed}}.
Such rapid and highly accurate recognition are the targets of modelling spike-based visual recognition.}
...
\DIFaddbegin \DIFadd{Inspired by biological studies on the visual ventral pathway, SNN models have successfully been adapted to computer vision recognition.} \cite{riesenhuber1999hierarchical} proposed a quantitative modelling framework for object recognition with position-, scale- and view-invariance.
...
\DIFaddbegin \DIFadd{Spike-Timing-Dependent Plasticity (STDP) as a biological learning process has been applied to vision tasks.
...
\DIFaddbegin \DIFadd{Apart from using spikes instead of the frame-based data used in conventional computer vision, new concerns on the latency and energy cost arise when evaluating neuromorphic vision in addition to recognition accuracy.
...
%	Other spike representation schemes may do a better job of preserving information from the original stimuli.
	...
	In order to reveal the advantages of neuromorphic engineering\DIFaddbegin \DIFadd{: high-accurate, rapid, energy-efficient recognition}, we need not only a spike-based dataset but also an appropriate evaluation methodology.
...
While in neuromorphic vision, MNIST is still an unsolved problem but a popular task among the research community as stated in Section~\ref{sec:intro}.}
