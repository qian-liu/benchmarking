...
Finally, we demonstrate the use of the dataset and the evaluation methodology using two SNN models to validate their classification capabilities and the performance of their hardware implementations.
...
A computer simulation achieved~93.3\% CA on MNIST and had the potential to be implemented using memristors~\citep{bill2014compound}. 
...
The recorded AER data contains consists of a list of events, each one containing the time stamp of a spike and the address of the neuron which generated the spike.
...
\begin{itemize}
	...
	to encourage research into fast recognition methods based on, for example, the primate visual pathway.
...
Python code is provide for converting from either file format to the other.
...
Since DoGs are used as a means to encode the image, and they do not form an orthogonal set of basis functions, the algorithm also performs a redundancy correction step.
...
\label{subsec_flash}
The purpose of including the subset with DVS-recorded flashing digits is to promote the application of Rank-Order-Coding to DVS output, and to accelerate fast on-set recognition by using just the early 30\% part of the spike train.
...
%	}
	...
	\caption{DVS sensor with flashing input. The bursty nature of the resulting spikes is illustrated, where most of the spikes occur in a 30~ms time slot. Blue is used for `ON' events and green for `OFF' events.}
	\label{fig:flash}
\end{figure}
...
This should include details of the way samples were presented: spiking rates, and biological time per testing sample.
...
The first case study is a simple two-layer network where the input neurons receive Poissonian spike trains from the dataset and form a fully connected network with the decision neurons.
...
Every testing image ($10,000$ images in total) is presented once and lasts 1~s with a silence of 200~ms between consecutive images.
...
The spike rates of the testing samples were also modified, ranging from 10 to $5,000$~Hz.
...
The variations in the total Sopbs ly in the firing rate of the output layers only, and the stronger connections lead to the higher firing rates.
