...
Because of the use of time dimension and the spike-based processing of SNN, there are more performance evaluation metrics involved except for classification accuracy: recognition latency and number of \DIFaddbegin \DIFadd{synaptic }\DIFaddend events.
...
The number of total \DIFaddbegin \DIFadd{synaptic }\DIFaddend events generated by a recognition task demonstrates the efficiency of a SNN model.
...
The number of \DIFaddbegin \DIFadd{synaptic }\DIFaddend events can be measured as Sopbs, synaptic operations per biological second.
...
\begin{table*}[hbt!]
	...
	\begin{center}
		...
		\begin{tabular}{ l l l l }
			...
			\begin{leftcell}{5cm} - \textbf{classification accuracy}\\ - \textbf{\textit{response latency}}\\ - \textbf{\textit{number of \DIFaddbegin \DIFadd{synaptic }\DIFaddend events}} \\ - \textit{biological testing time}\\ - \textit{input spiking rate}  \end{leftcell}% recognition
		\end{tabular}
		\egroup
	\end{center}
	\label{tb:model_eval}
...
Depending on how neurons, synapses and spike transmission are implemented, neuromorphic systems can be categorised as either analogue, digital, or mixed-mode analogue/digital. %VLSI circuits. 
...
The evaluation starts from the \DIFdelbegin \DIFdel{hardware-independent }\DIFdelend \DIFaddbegin \DIFadd{model-level }\DIFaddend side, focusing on the spike-based recognition analysis.
As mentioned in Section~\ref{subsec:model}, CA \DIFdelbegin \DIFdel{and  }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend response time (latency) \DIFaddbegin \DIFadd{and total number of synaptic events } \DIFaddend are the main concerns when assessing the recognition \DIFdelbegin \DIFdel{capability }\DIFdelend \DIFaddbegin \DIFadd{performance}\DIFaddend .
...
\DIFdelbegin \DIFdel{Furthermore, the latency of the output of the decision neurons is affected by both the spiking rate and connection weights.}\DIFdelend
...
\DIFaddbegin \DIFadd{As the default configuration of the SNN model, each input neuron connects to all of the 100 decision (output) neurons with both excitatory and inhibitory projections.
...
Although the difference of the Sopbs climbs to around 8~kHz, it is not obvious in the figure because the output firing rate are relatively low and so are the difference.}\DIFaddend
\begin{figure}[htb!]
	\centering
...
%	}
	...
	\caption{Accuracy\DIFdelbegin \DIFdel{and  }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend response time (latency\DIFaddbegin \DIFadd{) and synaptic event rate (Sopbs }\DIFaddend) change over test time and input firing rate per test image.
...
For the purpose of comparing the accuracy, simulation time, number of \DIFaddbegin \DIFadd{synaptic }\DIFaddend events and energy usage, different configurations have been tested on NEST (working on a PC with CPU: i5-4570 and 8G memory) and SpiNNaker.
...
The input rates in all of the tests are $5,000$~Hz, and each image is presented for 1~s\DIFaddbegin \DIFadd{ with 0.2~s silence in between}\DIFaddend.
...
The total number of \DIFaddbegin \DIFadd{synaptic }\DIFaddend events is around \DIFdelbegin \DIFdel{$5.01$M }\DIFdelend \DIFaddbegin \DIFadd{$4.17$M }\DIFaddend Sopbs, where only \DIFaddbegin \DIFadd{synaptic }\DIFaddend events is around \DIFdelbegin \DIFdel{$0.01$M }\DIFdelend \DIFaddbegin \DIFadd{7K }\DIFaddend spikes generated by the output layer. 
...
\DIFdelbegin \DIFdel{Given the number of }\DIFdelend \DIFaddbegin \DIFadd{Providing the number of synaptic }\DIFaddend events (Sopbs)\DIFaddbegin \DIFadd{ respectively for all the configurations }\DIFaddend and the simulation time, the energy per synaptic event on a hardware platform can be calculated (see Table~\ref{tbl:compare}): 
\DIFdelbegin \DIFdel{255}\DIFdelend \DIFaddbegin \DIFadd{306}\DIFaddend~nJ/SE running NEST on a PC, and \DIFdelbegin \DIFdel{82}\DIFdelend \DIFaddbegin \DIFadd{98}\DIFaddend~nJ/SE on SpiNNaker.
...
\begin{table}[h]
    ...
    \multicolumn{7}{c}{\vspace*{-3mm}}\\
		...
		& \multirow{2}{*}{$2,329.98$}
		& \multirow{2}{*}{$8,087.43$}
		& \multirow{2}{*}{$12,271.63$}
		& \multirow{2}{*}{$62,942.80$}
		\\	
...
Except for classification accuracy, the response latency and number of  \DIFaddbegin \DIFadd{synaptic }\DIFaddend events are the other specific evaluation metrics for spike-based processing.
