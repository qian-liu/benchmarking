...
%DIF DEL b0.tex            Mon Aug  8 15:43:13 2016
%DIF ADD NEbenchmark.tex   Tue Aug  9 15:14:05 2016
...
A detailed description of new spike-based learning rules will be a great contribution to the field due to the present \DIFdelbegin \DIFdel{short }\DIFdelend \DIFaddbegin \DIFadd{paucity }\DIFaddend of spatio-temporal learning algorithms.
...
%				$3,000,000$~s of training\\ 
			...
			\begin{mycell}{3.5cm} 92.99\%\\1~s per test\\\DIFdelbeginFL \DIFdelFL{200~ms silence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.2~s blank }\DIFaddendFL \\13.82~ms latency\\$4.17$M Sopbs\end{mycell}\\ % recognition
		\end{tabular}
		\egroup
	\end{center}
...
One test trial contains $10,000$ images in total and each image is presented once and lasts 1~s with a \DIFdelbegin \DIFdel{silence of 200~ms }\DIFdelend \DIFaddbegin \DIFadd{0.2~s blank period }\DIFaddend between consecutive images.
The output neuron with the highest firing rate determines which digit \DIFdelbegin \DIFdel{was recognised.
Taking }\DIFdelend \DIFaddbegin \DIFadd{is recognised.
With the standard training configuration, we compared the CA of different simulations of the same SNN model.
Using }\DIFaddend the trained weights from the NEST simulation, the accuracy of the recognition on NEST \DIFdelbegin \DIFdel{reaches }\DIFdelend \DIFaddbegin \DIFadd{reached }\DIFaddend 90.03\%\DIFdelbegin \DIFdel{with the standard configuration, while the result drops slightly to 89.97}%DIFDELCMD < \% %%%
\DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{, and this accuracy was also achieved on }\DIFaddend SpiNNaker.
When \DIFaddbegin \DIFadd{the network was }\DIFaddend both trained and tested on SpiNNaker the recognition accuracy \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend 87.41\%\DIFdelbegin \DIFdel{, and with these weights used in NEST the result is }\DIFdelend \DIFaddbegin \DIFadd{.
Using these weights in NEST yielded a similar result (}\DIFaddend 87.25\%\DIFdelbegin \DIFdel{. 
The CA drops less than $0.06\%$ on SpiNNaker compared to NEST due to }\DIFdelend \DIFaddbegin \DIFadd{). 
The reduction in CA using the SpiNNaker trained weights was due to precision loss caused by }\DIFaddend the limited fast memory and the necessity for fixed-point arithmetic \DIFdelbegin \DIFdel{on SpiNNaker }\DIFdelend to ensure real-time operation.
...
The input rates in all of the tests are $5,000$~Hz, and each image is presented for 1~s with \DIFaddbegin \DIFadd{a }\DIFaddend 0.2~s \DIFdelbegin \DIFdel{silence in between }\DIFdelend \DIFaddbegin \DIFadd{blank period between consecutive images during which the model receives no input}\DIFaddend .
