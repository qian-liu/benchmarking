...
%DIF ADD NEbenchmark.tex   Mon Aug  8 18:30:01 2016
...
Taking the trained weights from the NEST simulation, the accuracy of the recognition on NEST reaches 90.03\% with the standard configuration, \DIFdelbegin \DIFdel{while the result drops slightly to 89.97}%DIFDELCMD < \% %%%
\DIFdel{using SpiNNaker. }\DIFdelend \DIFaddbegin \DIFadd{and SpiNNaker achieves the same result. %DIF >  while the result drops slightly to 89.97\% using SpiNNaker.
}\DIFaddend When both trained and tested on SpiNNaker the recognition accuracy is 87.41\%, and with these weights used in NEST the result is 87.25\%. 
The CA \DIFdelbegin \DIFdel{drops less than $0.06\%$ }\DIFdelend \DIFaddbegin \DIFadd{differs }\DIFaddend on SpiNNaker compared to NEST due to the limited fast memory and the necessity for fixed-point arithmetic on SpiNNaker to ensure real-time operation.
