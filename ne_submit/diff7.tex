...
%DIF DEL clean.tex         Sun Aug  7 09:57:49 2016
%DIF ADD NEbenchmark.tex   Sun Aug  7 09:51:29 2016
...
Within this field, the primate visual pathway and its hierarchical organisation have been extensively studied\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend Spiking Neural Networks (SNNs), inspired by the understanding of observed biological structure and function, have been successfully applied to visual recognition and classification tasks.
...
Neuromorphic sensors such as silicon retinas are able to feed such \DIFdelbegin \DIFdel{a mobile system }\DIFdelend \DIFaddbegin \DIFadd{mobile systems }\DIFaddend with real-time visual stimuli.
...
In this paper we first propose an initial NE (Neuromorphic Engineering) dataset based on standard computer vision \DIFdelbegin \DIFdel{benchmarks and }\DIFdelend \DIFaddbegin \DIFadd{benchmarksand that }\DIFaddend uses digits from the MNIST database.
...
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. in 200~ms in monkeys~\citep{fabre1998rapid}, even with short \DIFdelbegin \DIFdel{appearance }\DIFdelend \DIFaddbegin \DIFadd{presentations }\DIFaddend (less than 100~ms) of the target objects~\citep{keysers2001speed}.
...
%	Other spike representation schemes may do a better job of preserving information from the original stimuli.
	...
	To reveal the \DIFdelbegin \DIFdel{advantages of neuromorphic engineering: highly 	accurate, rapid, }\DIFdelend \DIFaddbegin \DIFadd{accuracy, speed, and }\DIFaddend energy-efficient recognition \DIFaddbegin \DIFadd{of neuromorphic approaches}\DIFaddend , we need not only a spike-based dataset but also an appropriate evaluation methodology.
	...
	However, these must represent an appropriate degree of \DIFdelbegin \DIFdel{challenge}\DIFdelend \DIFaddbegin \DIFadd{difficulty}\DIFaddend : a too-easily-solved problem turns into a tuning competition, while a problem that is too difficult will not yield meaningful assessment.
...
This algorithm models the \DIFdelbegin \DIFdel{foveal pit region}\DIFdelend \DIFaddbegin \DIFadd{foveola}\DIFaddend , the highest resolution area of the retina, with four ganglion cell layers each with a different scale of centre-surround receptive field~\citep{kolb2003retina}. To simulate these layers two steps are required: the first consists of four discrete 2D convolutions; the second removes redundant information produced in the first step. During the first step, the centre-surround behaviour of the ganglion cells is modelled using Difference of Gaussians~(DoG) kernel for convolution. 
\begin{equation}
\label{eq-dog}
...
\label{sec:eval}
As a result of the spike-based processing used in SNN models, new concerns \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{about }\DIFaddend the latency and energy cost arise over performance assessment.
...
Because SNNs introduce the time dimension and spike-based processing, additional performance metrics become relevant in addition to classification accuracy: \DIFdelbegin \DIFdel{these are }\DIFdelend recognition latency and the number of synaptic events.
...
A small \DIFdelbegin \DIFdel{total number of }\DIFdelend \DIFaddbegin \DIFadd{number of total }\DIFaddend synaptic events generated by a recognition task indicates the efficiency of the SNN model.
...
Fewer spike events \DIFdelbegin \DIFdel{implies }\DIFdelend \DIFaddbegin \DIFadd{imply }\DIFaddend lower overall neural activity and lower energy consumption.
...
The main characteristics include the \DIFaddbegin \DIFadd{network }\DIFaddend topology and the neural and synaptic models.
The network topology defines the number of neurons used for each layer and the connections between layers and neurons.
It is essential to state the types of neural and synaptic model (e.g. current-based LIF neuron) utilised in the network and the parameters configuring them, because neural activities differ \DIFdelbegin \DIFdel{greatly }\DIFdelend \DIFaddbegin \DIFadd{significantly }\DIFaddend between configurations.
Any non-neural classifier, sometimes added to aid the design \DIFaddbegin \DIFadd{or }\DIFaddend enhance the output of the network, must also be specified.
...
Specifying the learning algorithm with its mechanism (supervised, semi-supervised and unsupervised) helps the reader \DIFdelbegin \DIFdel{undersdand }\DIFdelend \DIFaddbegin \DIFadd{understand }\DIFaddend the core features of the model.
A detailed description of new spike-based learning rules will be a great contribution to the field due to the present \DIFdelbegin \DIFdel{lack }\DIFdelend \DIFaddbegin \DIFadd{short }\DIFaddend of spatio-temporal learning algorithms.
...
This includes details of the way samples \DIFdelbegin \DIFdel{were presented }\DIFdelend \DIFaddbegin \DIFadd{are presented to the network}\DIFaddend : spiking rates, and biological time per test sample.
...
Following to the formatted evaluation as in Table~\ref{tb:model_eval}, Table~\ref{tb:software_comparison} lists a few SNN models of MNIST classification, although some \DIFdelbegin \DIFdel{detailed description is }\DIFdelend \DIFaddbegin \DIFadd{details are }\DIFaddend missing. 
      ...
      \begin{minipage}{\textwidth}
        ...
        \begin{savenotes}
  		...
  		\begin{tabular}{l c c c c c c}
  			...
  			\begin{mycell}{2.0cm}11- to 14-bit \DIFdelbeginFL \DIFdelFL{synapse}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapses}\DIFaddendFL \end{mycell} & 
  			...
  			\begin{mycell}{2.2cm}122 bits \\params \& states,
  				%       	 per neuron
  				\\4-bit/\\4 values\\\DIFdelbeginFL \DIFdelFL{synapse 
  				}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapses 
  				}\DIFaddendFL %\\(4 signed int + on/off state)
          \footnote[1]{We consider them 4-bit \DIFdelbeginFL \DIFdelFL{synapse }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapses }\DIFaddendFL because it is only possible to choose between 4 different signed integers and whether the synapse is active or not.}
  			\end{mycell}& 
  			...
  			\begin{mycell}{2.0cm}13-bit shared \\ \DIFdelbeginFL \DIFdelFL{synapse}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapses}\DIFaddendFL \end{mycell} &
  			\begin{mycell}{2.0cm}Real-time\end{mycell} &
  			...
  			\begin{mycell}{2.0cm}Fixed models,\\Config params\end{mycell}& 
  			\begin{mycell}{2.0cm}Fixed rule\end{mycell}& 
  			\begin{mycell}{2.0cm}4-bit/\\16 values\\\DIFdelbeginFL \DIFdelFL{synapse}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapses}\DIFaddendFL \end{mycell}& 
...
\DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{Moreover, }\DIFaddend the requirement for the models to run in real time leads to constraints on the complexity of model that can be supported.
...
Considering the various neural and synaptic models, plasticity learning rules and lengths of axonal delays, a programmable platform offers flexibility to support diverse SNNs while a hard-wired system supporting only specific models \DIFdelbegin \DIFdel{wins for }\DIFdelend \DIFaddbegin \DIFadd{is advantageous due to }\DIFaddend its energy-efficiency and simpler design and implementation.
The classification accuracy of an SNN running on a hardware system can be different from the software simulation, since hardware implementations may impose limits on the precision used for the membrane \DIFdelbegin \DIFdel{potential }\DIFdelend \DIFaddbegin \DIFadd{potentials }\DIFaddend of neurons (for the digital platforms) and the synaptic weights.
...
Due to the limited hardware resources, simulation time may \DIFdelbegin \DIFdel{, likewise, }\DIFdelend accelerate or slow down according to the network topology and spike dynamics.
...
One test trial contains $10,000$ images in total and each image is presented once and lasts 1~s with a silence of 200~ms between consecutive images.
...
\DIFdelbegin \DIFdel{As }\DIFdelend \DIFaddbegin \DIFadd{At }\DIFaddend the default configuration of the SNN model, each input neuron connects to all of the 100 decision (output) neurons with both excitatory and inhibitory projections.
...
On the other hand, SpiNNaker works in \DIFdelbegin \DIFdel{real (biologicallyreal) }\DIFdelend \DIFaddbegin \DIFadd{(biologically) real }\DIFaddend time and the simulation time becomes shorter than the NEST simulation when $1,000$ patterns per digit ($1,000$ decision neurons per digit) are used.
...
A built-in \DIFdelbegin \DIFdel{Anduino board read out }\DIFdelend \DIFaddbegin \DIFadd{Arduino board provided a measurement read out of }\DIFaddend the power usage of the chips.
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{This }\DIFaddend result provides a baseline for comparison with other SNN models and neuromorphic hardwares, and no optimisation is applied.
\begin{table}[h]
...
\begin{figure}[hbt!]
	...
	\caption{
	Total energy consumption \DIFaddbeginFL \DIFaddFL{(black) }\DIFaddendFL and number of SE per second \DIFaddbeginFL \DIFaddFL{(blue) }\DIFaddendFL of a spiking DBN running on a single SpiNNaker chip as a function of the total input firing rate.}
...
The \DIFaddbegin \DIFadd{proposed }\DIFaddend complementary evaluation methodology is essential to assess both the model-level and hardware-level performance of SNNs.
...
\DIFdelbegin \DIFdel{In all}\DIFdelend \DIFaddbegin \DIFadd{Overall}\DIFaddend , it is impossible for the dataset proposers to provide enough datasets, converting methods and benchmarking results, thus we encourage other researchers to contribute to the dataset.
Researchers can contribute their data to the dataset, allowing future comparisons using the same data source\DIFdelbegin \DIFdel{;
they can }\DIFdelend \DIFaddbegin \DIFadd{.
They can also }\DIFaddend share their spike conversion algorithms by generating datasets to promote the corresponding recognition methods\DIFdelbegin \DIFdel{;
and neuromorphic }\DIFdelend \DIFaddbegin \DIFadd{.
Neuromorphic }\DIFaddend hardware owners are welcome to provide benchmarking results to compare their system's performance.
