...
\DIFdelbegin \DIFdel{Within }\DIFdelend \DIFaddbegin \DIFadd{Inside }\DIFaddend the field, the visual pathway and its hierarchical organisation have been extensively studied within the primate brain.
...
In addition, implementations on \DIFdelbegin \DIFdel{neuromorhpic hardware have made }\DIFdelend \DIFaddbegin \DIFadd{neuromorphic hardware have enabled }\DIFaddend large-scale networks \DIFaddbegin \DIFadd{to }\DIFaddend run in (or even faster than) real time, and \DIFaddbegin \DIFadd{made it }\DIFaddend accessible on mobile robots.
...
We propose that a large dataset of spike-based visual stimuli is needed to provide \DIFdelbegin \DIFdel{a baseline for comparisons on SNN models and algorithms, and some benchmarking network models are }\DIFdelend \DIFaddbegin \DIFadd{meaningful comparisons with unified data, and its corresponding evaluation method is }\DIFaddend also required to \DIFdelbegin \DIFdel{validate the accuracy and cost of these neuromorphic hardware platforms.
}\DIFdelend \DIFaddbegin \DIFadd{estimate the performance of SNN models and their hardware implementations.}\DIFaddend 
%DIF > \replaced{We propose that a large dataset of spike-based visual stimuli is needed to provide meaningful comparisons with unified data, and its corresponding evaluation method is also required to estimate the performance of SNN models and their hardware implementations.}{We propose that a large dataset of spike-based visual stimuli is needed to provide a baseline for comparisons on SNN models and algorithms, and some benchmarking network models are also required to validate the accuracy and cost of these neuromorphic hardware platforms.}
%DIF > We propose that a large dataset of spike-based visual stimuli is needed to provide a baseline for comparisons on SNN models and algorithms, and some benchmarking network models are also required to validate the accuracy and cost of these neuromorphic hardware platforms.



First of all, an initial NE (Neuromorphic Engineering) dataset \DIFdelbegin \DIFdel{of input stimuli }\DIFdelend \DIFaddbegin \DIFadd{is presented, which is }\DIFaddend based on standard computer vision benchmarks \DIFdelbegin \DIFdel{consisting of digits (}\DIFdelend \DIFaddbegin \DIFadd{and consists of digits }\DIFaddend from the MNIST database\DIFdelbegin \DIFdel{) is presented according }\DIFdelend \DIFaddbegin \DIFadd{. This dataset is in accordance }\DIFaddend to the current research on spike-based image recognition.
\DIFdelbegin \DIFdel{Within this dataset, all images are centre aligned and having similar scale.
...
}\DIFdelend %DIF > First of all, an initial NE (Neuromorphic Engineering) dataset of input stimuli based on standard computer vision benchmarks consisting of digits (from the MNIST database) is presented according to the current research on spike-based image recognition.
%DIF > Within this dataset, all images are centre aligned and have similar scale.
%DIF > We describe how we intend to expand this dataset to fulfil the needs of upcoming research problems.
%DIF > For instance, the data should provide cases to measure position-, scale-, and viewing-angle invariance.
%DIF > The data are in Address-Event Representation (AER) format which is widely used in the neuromorphic engineering field unlike conventional images.
...
As a consequence, a new series of spike-based vision datasets is now needed to quantitatively measure progress within this rapidly advancing field and to provide fair competition resources for researchers.
	...
	%DIF > Benchmarking neuromorphic hardware with various network models will reveal the advantages and disadvantages of different platforms.
	\DIFaddbegin \DIFadd{Running various SNN benchmarks reveals the feasibility of a hardware platform to support model-specific configurations on network size, topology, neural and synaptic models, learning algorithms and etc.
	...
	In this paper we propose a large dataset of spike-based visual stimuli, NE, and its complementary evaluation methodology.
	The dataset expands and evolves as research develops and new problems are introduced.}%DIFDELCMD < 
	...
	\DIFdelbegin \DIFdel{The sub-datasets and their generation methods are described in detail in Section~\ref{sec:data}.
...
The spike trains are in the format of \DIFdelbegin \DIFdel{AER }\DIFdelend \DIFaddbegin \DIFadd{Address-Event Representation~(AER)~\mbox{%DIFAUXCMD
		\citep{mahowald1992vlsi} }%DIFAUXCMD
}\DIFaddend data, which could easily be used in both event-driven computer simulations and neuromorphic systems.
\DIFaddbegin \DIFadd{The AER was originally proposed as a communication protocol using time-multiplexing to replace the connection wires for each connected neuron pair.
...
}\DIFaddend With the NE dataset we hope:
\begin{itemize}
...
The original \DIFdelbegin \DIFdel{MNIST dataset is }\DIFdelend \DIFaddbegin \DIFadd{dataset was }\DIFaddend downloaded from the \DIFaddbegin \DIFadd{MNIST Database of Handwritten Digits~\mbox{%DIFAUXCMD
		\citep{lecun1998gradient}  }%DIFAUXCMD
}\DIFaddend website\footnote{http://yann.lecun.com/exdb/mnist/}\DIFdelbegin \DIFdel{of THE MNIST DATABASE of handwritten digits~\mbox{%DIFAUXCMD
	\citep{lecun1998gradient}  }%DIFAUXCMD
}\DIFdelend .
The NE15-MNIST is \DIFdelbegin \DIFdel{converted into a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend spiking version of the original dataset\DIFdelbegin \DIFdel{consisting }\DIFdelend \DIFaddbegin \DIFadd{, the converted set consists }\DIFaddend of four subsets which were generated for different purposes:
\begin{itemize}
	\item \textit{Poissonian}
	to benchmarking existing methods of rate-based spiking models.
	\item \textit{FoCal\DIFaddbegin \DIFadd{~(Filter Overlap Correction ALgorithm)}\DIFaddend }
...
Two file formats are supported in the dataset: jAER format~\citep{delbruck2008frame} (.dat or .aedat), and binary file in NumPy\DIFdelbegin \DIFdel{.npy format.
	...
	The spikes }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
		\citep{numpyPython} }%DIFAUXCMD
	(.npy) format.
	%DIF > The AER interface has been widely used in neuromorphic systems, especially for vision sensors.
	%DIF > The spikes are encoded as time events with corresponding addresses to convey information.
	The spikes }\DIFaddend in jAER format, both recorded from a DVS retina and artificially generated, can be displayed \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend jAER software.
...
The \DIFdelbegin \DIFdel{other format of spikes }\DIFdelend \DIFaddbegin \DIFadd{second spike-based format }\DIFaddend used is a list of spike source arrays in PyNN~\citep{davison2008pynn}, a description language for building spiking neuronal network models.
Python code for converting \DIFaddbegin \DIFadd{from }\DIFaddend one file format to \DIFdelbegin \DIFdel{and from the other is also provided.
}\DIFdelend \DIFaddbegin \DIFadd{the other and vice versa is provided.
The duration of artificially generated data could be configured using the Python code provided, while the recorded data aiming at different tasks vary in duration: 1~s for the flashing input, and 3.2 to 3.4~s for the moving input.}\DIFaddend 
...
{\DIFdelbegin \DIFdel{Filter overlap Correction algorithm}\DIFdelend \DIFaddbegin \DIFadd{FoCal algorithm~\mbox{%DIFAUXCMD
			\citep{sen2009evaluating}}%DIFAUXCMD
	}\DIFaddend }\DIFdelbegin \DIFdel{or FoCal ~\mbox{%DIFAUXCMD
	\citep{sen2009evaluating}}%DIFAUXCMD
}\DIFdelend .
It models the foveal pit region, the highest resolution area of the retina, with four ganglion cell layers that \DIFdelbegin \DIFdel{show }\DIFdelend \DIFaddbegin \DIFadd{present }\DIFaddend a centre-surround behaviour~\citep{kolb2003retina}. In order to simulate these layers \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{two steps are required, the first one consists of }\DIFaddend four discrete 2D convolutions\DIFdelbegin \DIFdel{are performed. The }\DIFdelend \DIFaddbegin \DIFadd{; the second one removes redundant information produced by the first step. During the first step, the }\DIFaddend centre-surround behaviour of the ganglion cells is modelled using Differences of Gaussians~(DoG) \DIFaddbegin \DIFadd{kernels for convolution}\DIFaddend . 
\begin{equation}
\label{eq-dog}
...
is inversely proportional to a spike emission time \DIFaddbegin \DIFadd{with respect to the presentation of the image }\DIFaddend (i.e. the higher the pixel value, the sooner the spike will be sent out.)
...
\end{figure}
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Since DoGs where used as a means to encode the image, and they are not an orthogonal basis, the }\DIFaddend algorithm also performs a redundancy correction step, it does so by 
adjusting the convolved image's pixel value according to the correlation 
between convolution kerne	ls (Alg.~\ref{code-focal-corr}).
\begin{algorithm}[h]
	\caption{FoCal, \DIFdelbegin \DIFdel{Part 2}\DIFdelend \DIFaddbegin \DIFadd{redundancy correction}\DIFaddend }
...
\label{sec:eval}
\DIFaddbegin \DIFadd{Due to the spike-based processing of SNN models, there are new concerns over the performance assessment.
...
}\DIFaddend A complementary evaluation methodology is essential to provide common metrics and assess both the model-level and hardware-level performance.
\begin{table*}[hbt!]
	...
	\begin{center}
		...
		\begin{tabular}{ l c c c c }
			...
			\begin{mycell}{3.5cm}  \DIFaddbegin \DIFadd{92.99\%\\1~s per test\\13.82~ms latency\\$5.01$M Sopbs}\DIFaddend\end{mycell}\\ % recognition
		\end{tabular}
		\egroup
	\end{center}
...
\DIFdelbegin \DIFdel{First of all it is desirable for researchers to specify whether they add any }\DIFdelend \DIFaddbegin \DIFadd{Because of the use of time dimension and the spike-based processing of SNN, there are more performance evaluation metrics involved except for classification accuracy: recognition latency and number of events.
	%DIF > Both of the metrics are proposed according to the biological aspects.
	The recognition latency measures how fast spikes are conveyed through layers of network to trigger recognition neurons.
	\mbox{%DIFAUXCMD
		\cite{dicarlo2012does} }%DIFAUXCMD
	considers the rapid ($<$200~ms) and accurate vision recognition in the brain as the essential problem of object recognition.
	...
	A spike event is a synaptic operation evoked when one action potential hitting one synapse~\mbox{%DIFAUXCMD
		\citep{sharp2012power}}%DIFAUXCMD
	.
...
\DIFadd{Besides of the SNN evaluation metrics, a sufficient description of a network model is required so that other researchers can reproduce it and compare with other models.
	First of all, the input of a SNN should be specified.
	The description includes the transformation method of converting raw images to spike trains, and the }\DIFaddend preprocessing either to images or spikes.
Filtering the raw \DIFdelbegin \DIFdel{input }\DIFdelend \DIFaddbegin \DIFadd{image }\DIFaddend may ease the classification/recognition task while adding noise may require stronger robustness of the model.
Secondly, as with the evaluation on conventional artificial neural networks, a description of the network characteristics is \DIFdelbegin \DIFdel{most welcome }\DIFdelend \DIFaddbegin \DIFadd{supposed to be provided }\DIFaddend since it is the basis for the overall performance.
\DIFdelbegin \DIFdel{Furthermore, sharing the designs may inspire }\DIFdelend \DIFaddbegin \DIFadd{Sharing the designs not only makes the model reproducible but also inspires }\DIFaddend fellow scientists to bring new points of view to the problem and generate a positive feedback loop where everybody wins.
The \DIFdelbegin \DIFdel{network description should }\DIFdelend \DIFaddbegin \DIFadd{main characteristics }\DIFaddend include the topology, and the neural and synaptic models.
The network topology defines the number of neurons used for each layer, and the connections between layers and neurons.
\DIFdelbegin \DIFdel{Some researchers make use of extra non-neural classifiers, sometimes to aid the design, others to enhance the output of the network.
	Any particulars on this subject are greatly appreciated.
}\DIFdelend It is essential to state the \DIFdelbegin \DIFdel{type }\DIFdelend \DIFaddbegin \DIFadd{types }\DIFaddend of neural and synaptic \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{models }\DIFaddend (e.g. current-based LIF neuron) \DIFdelbegin \DIFdel{exploited }\DIFdelend \DIFaddbegin \DIFadd{utilised }\DIFaddend in the network and the parameters configuring them, because neural activities differ greatly between various configurations.
\DIFaddbegin \DIFadd{Any extra non-neural classifier, sometimes to aid the design, others to enhance the output of the network, is also required to represent the model.}

\DIFaddend Thirdly, the learning procedure determines the recognition capability of a network model.
...
Similarly, the biological time of training decides the amount of information provided \DIFdelbegin \DIFdel{.}
\DIFdelend \DIFaddbegin \DIFadd{for training a SNN.
}\DIFaddend Finally in the testing phase\DIFdelbegin \DIFdel{where performance evaluation takes place, specific measurements of SNN models are essential in addition to recognition accuracy}\DIFdelend \DIFaddbegin \DIFadd{, besides the performance evaluation metrics stated above, specific configurations of the input spikes are also essential}\DIFaddend .
It should include details of the way samples were presented: \DIFdelbegin \DIFdel{event }\DIFdelend \DIFaddbegin \DIFadd{spiking }\DIFaddend rates, and biological time per testing sample.
The combination of these two factors determines how much information is presented to the network.
\DIFdelbegin \DIFdel{An important performance metric is the response time (latency) of an SNN model .
	...
	Work }\DIFdelend %DIF > An important performance metric is the response time (latency) of an SNN model.
%DIF > A faster model is more suitable for real-time recognition systems such as neuromorphic robotics.
%DIF > A commonly reported characteristic is the accuracy of the network, perhaps adding remarks on how these scores are obtained could help to unify criteria and ease comparison.

\DIFaddbegin 

\DIFadd{As a summery, the description of a SNN model is suggested as Table~\ref{tb:model_eval} where the specific description for SNN is written with italic and the performance evaluation metrics are in bold.
	Some works }\DIFaddend on SNN-based classifications of MNIST are listed in Table~\ref{tb:software_comparison}\DIFdelbegin \DIFdel{and evaluated on the proposed metrics}\DIFdelend \DIFaddbegin \DIFadd{.
	Most of the reports did not provide sufficient description of the networks and the models were not completely evaluated as we propose}\DIFaddend .

\DIFaddbegin \begin{table*}[hbt!]
	\caption{\DIFaddFL{SNN descriptions in model level}}
	\begin{center}
		\bgroup
		\def\arraystretch{1.5}
		\begin{tabular}{ l l l l }
			\begin{mycell}{3.7cm}\DIFaddFL{Input}\end{mycell} & 
			\begin{mycell}{2.5cm} \DIFaddFL{Network}\end{mycell} & 
			\begin{mycell}{4cm} \DIFaddFL{Training }\end{mycell} & 
			\begin{mycell}{4.5cm} \DIFaddFL{Recognition }\end{mycell} \\
			\hline
			
			\begin{leftcell}{3.7cm} \DIFaddFL{- }\textit{\DIFaddFL{converting methods}}\\\DIFaddFL{- preprocessing }\end{leftcell} & %DIF >  preprocessing
			\begin{leftcell}{2.5cm} \DIFaddFL{- topology}\\\DIFaddFL{- }\textit{\DIFaddFL{neuron and synaptic type}} \end{leftcell}&  %DIF >  network
			\begin{leftcell}{4cm} \DIFaddFL{- supervised or not}\\\DIFaddFL{- }\textit{\DIFaddFL{learning rule}} \\ \DIFaddFL{- }\textit{\DIFaddFL{biological training time}}\end{leftcell}&  %DIF >  training
			\begin{leftcell}{4.5cm} \DIFaddFL{- }\textbf{\DIFaddFL{classification accuracy}}\\ \DIFaddFL{- }\textbf{\textit{\DIFaddFL{response latency}}}\\ \DIFaddFL{- }\textbf{\textit{\DIFaddFL{number of events}}} \\ \DIFaddFL{- }\textit{\DIFaddFL{biological testing time}}\\ \DIFaddFL{- }\textit{\DIFaddFL{input spiking rate}}  \end{leftcell}%DIF >  recognition
		\end{tabular}
		\egroup
	\end{center}
	\label{tb:model_eval}
...
Depending on how neurons, synapses and spike transmission are implemented\DIFaddbegin \DIFadd{, }\DIFaddend neuromorphic systems can be categorised as either analogue, digital, or mixed-mode analogue/digital\DIFdelbegin \DIFdel{VLSI circuits. }\DIFdelend \DIFaddbegin \DIFadd{. %DIF > VLSI circuits. 
...
%Qian Liu modified
\DIFdelbegin \DIFdel{The metric proposed in }\DIFdelend Table~\ref{tb:hardware_comparison} attempts to \DIFdelbegin \DIFdel{expose the advantages and disadvantages of different neuromorphic hardware thus to find out the network properties each platform is suited to}\DIFdelend \DIFaddbegin \DIFadd{describe the neuromorphic hardware platforms in different aspects of SNN simulation}\DIFaddend .
...
The classification accuracy of a SNN running on a hardware system can be different from the software simulation, since hardware \DIFdelbegin \DIFdel{implementation }\DIFdelend \DIFaddbegin \DIFadd{implementations may impose }\DIFaddend limits on the precision used for the membrane potential of neurons (for the digital platforms) and the synaptic weights.
\DIFdelbegin \DIFdel{Thus comparison metrics is supposed to include precision as a major assessment of the system performance.
}\DIFdelend %DIF > Thus comparison metrics is supposed to include precision as a major assessment of the system performance.
...
\DIFdelbegin \DIFdel{However, due to the limitation of hardware resources simulation time may accelerate or slow down according to the network topology and spike dynamics.
}\DIFdelend Also finer time resolution plays an important role in precision sensitive neural models or in sub-millisecond tasks~\citep{lagorce2015breaking}.
%Qian Liu done
...
Merolla et al. proposed the number of synaptic operations per \DIFaddbegin \DIFadd{second per }\DIFaddend Watt (Sops/W)~\citep{merolla2014million}.
These two measurements are the same presentations of energy use of synaptic events, since J/SE$\times$Sops/W = \DIFdelbegin \DIFdel{1 s. 
}\DIFdelend \DIFaddbegin \DIFadd{1.}\DIFaddend 

%Qian Liu modified
\DIFdelbegin \DIFdel{For a particular SNN application or benchmark, the scalability and programmability will determine whether the network is able to run on a platform.
	The system performance will be assessed on the }\DIFdelend \DIFaddbegin \DIFadd{However, the reported simulation time and energy use is under different SNN models making the comparisons fragile.
	...
	The proposed evaluation metrics consist of the feasibility, classification }\DIFaddend accuracy, simulation time and energy use\DIFdelbegin \DIFdel{running the  network . 
...
Similarly, energy cost varies with different networks and even neural and synaptic models.}\DIFaddend 
%DIF > The system performance will be assessed on the accuracy, simulation time and energy use running the network. 
%DIF > Table~\ref{tb:hardware_comparison} aims to summarise the aforementioned hardware comparison metrics.

\section{Results}
\label{sec:test}
In this section, we present two recognition SNN models working on the Poissonian subset of the NE15-MNIST dataset.
Their network components, training and testing methods are described according to \DIFdelbegin \DIFdel{the evaluation methodology stated above.
	The specific spike-based evaluations on input event rates and}\DIFdelend \DIFaddbegin \DIFadd{Section~\ref{subsec:model}.
	The recognition result is evaluated on (all}\DIFaddend /\DIFdelbegin \DIFdel{or responding latency are also provided.
}\DIFdelend \DIFaddbegin \DIFadd{some of) the metrics proposed: classification accuracy, response latency and number of events.
%DIF > The specific spike-based evaluations on input event rates and/or responding latency are also provided. 
}\DIFaddend Meanwhile, as tentative benchmarks the models are implemented on SpiNNaker to assess the \DIFaddbegin \DIFadd{hard-level }\DIFaddend performance against software simulators.
...
The first case study is a simple two-layered network where the input neurons receive Poissonian presented spike trains from the dataset and form \DIFdelbegin \DIFdel{an FC }\DIFdelend \DIFaddbegin \DIFadd{a fully connected }\DIFaddend network with the decision neurons.
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{There is at least one decision neuron per digit to classify a test input.
	The neuron with highest output firing rate classifies a test image into the digit it represents.
	The }\DIFaddend model utilises LIF neurons, and the parameters are all with biological means, see the listed values in Table~\ref{tbl:pynnSetting}.
...
\DIFdelbegin \DIFdel{So the images }\DIFdelend \DIFaddbegin \DIFadd{K-means clustering separates a set of data points into K subsets (clusters) according to the Euclidean distance.
	...
	Then the images }\DIFaddend in a certain subclass are used to train \DIFdelbegin \DIFdel{one }\DIFdelend \DIFaddbegin \DIFadd{a template embedded in the synaptic weights to the }\DIFaddend corresponding decision neuron.
The firing rates of the input neurons are assigned linearly according to their intensities and \DIFdelbegin \DIFdel{normalised with a total }\DIFdelend \DIFaddbegin \DIFadd{the total firing rate of all the 28$\times$28 input neurons is normalised with $2,000$~Hz, e.g. the summation of the }\DIFaddend firing rate of \DIFaddbegin \DIFadd{all the input neurons is }\DIFaddend $2,000$~Hz.
...
%	}
	...
	\DIFaddendFL \caption{Accuracy and response time (latency) change over test time and input firing rate per \DIFdelbeginFL \DIFdelFL{testing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{test }\DIFaddendFL image.
		\DIFaddbeginFL \DIFaddFL{The test time is the duration of the presence of a single test image, and the input firing rate is the summation of all the input neurons.
		}\DIFaddendFL Original trained weights are used (circles in blue) as well as the scaled up ($\times10$) weights (triangles in black). }
	\label{fig:assess}
\end{figure}
...
For the purpose of comparing the accuracy, simulation time\DIFaddbegin \DIFadd{, number of events }\DIFaddend and energy usage, different configurations have been tested on NEST (working on a PC with CPU: i5-4570 and 8G memory) and SpiNNaker\DIFdelbegin \DIFdel{, see }\DIFdelend \DIFaddbegin \DIFadd{.
	The same experiment has been run 4~times with different random seeds, the average performance estimation is listed in }\DIFaddend Table~\ref{tbl:compare}.
...
\DIFdelbegin \DIFdel{The recognition accuracies differ in a range of $\pm0.5\%$ between NEST and SpiNNaker }\DIFdelend \DIFaddbegin \DIFadd{Thus the network size vary according to the number of neurons in the decision layer where each neuron represents a subclass of a digit and the template is embedded in the synaptic weights connecting from the input layer to the decision neuron.
	The CA drops less than $0.06\%$ on SpiNNaker comparing to NEST }\DIFaddend due to the limited fast memory and the necessity for fixed-point arithmetic on SpiNNaker to ensure real-time operation.
...
\DIFdelbegin \DIFdel{NEST was run fully active on a single core which cost 21~W of power usage }\DIFdelend %DIF > NEST was run fully active on a single core, and only OS related operations were on the other cores.
\DIFaddbegin \DIFadd{The power usage is estimated by subtracting the power usage running the simulation and cost by idle OS operations}\DIFaddend .
The energy use can be calculated as the product of the simulation time and the power use.
Even with the smallest network, SpiNNaker wins in the energy cost comparison, see \DIFdelbegin \DIFdel{Fig.}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:energy}.
Among different network configurations, the \DIFdelbegin \DIFdel{network }\DIFdelend \DIFaddbegin \DIFadd{model }\DIFaddend of 500 decision neurons (50 clusters per digit) reaches the highest recognition rate \DIFdelbegin \DIFdel{.
	%DIF < The recognition accuracy reaches the highest (92.98\%) when 50 patterns are trained per digit.
	The network achieved a CA of 92.98\% and average latency }\DIFdelend of \DIFdelbegin \DIFdel{10.70~ms, and the simulation costs SpiNNaker 0.41~W on power use and $4,920$~J on energy use.
} \DIFdelend \DIFaddbegin \DIFadd{92.99\% in average having the latency of about 13.82~ms.
...
Given the number of events (Sopbs) and the simulation time, the energy per synaptic event on a hardware platform can be calculated (see Table~\ref{tbl:compare}): 
255~nJ/SE running NEST on a PC, and 82~nJ/SE on SpiNNaker.
...
% about oneils model 
\citet{o2013real} proposed a method to map off-line trained DBNs into a spiking neural networks and take advantage of the real-time performance and energy efficiency of neuromorphic platforms. This led initially to an implementation on an event-driven Field-Programmable Gate Array (FPGA) called Minitaur~\citep{neil2014minitaur} and then on the SpiNNaker platform~\citep{Stromatias2015scalable}. For this work we used \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{the identical }\DIFaddend off-line trained\footnote{\url{https://github.com/dannyneil/edbn/}} spiking DBN \DIFdelbegin \DIFdel{with a }\DIFdelend \DIFaddbegin \DIFadd{as presented by \mbox{%DIFAUXCMD
		\citet{o2013real}}%DIFAUXCMD
	, which comprises 784 neurons for the input layer, two hidden layers with 500 neurons each, and an output layer with 10 neurons. This is abbreviated as }\DIFaddend 784-500-500-10 \DIFdelbegin \DIFdel{network topology}\DIFdelend \DIFaddbegin \DIFadd{architecture}\DIFaddend . Simulations take place on a software spiking neural network simulator named Brian~\citep{goodman2008brian} and results are verified on the SpiNNaker platform.

\subsubsection{Training}

DBNs consist of stacked Restricted Boltzmann Machines (RBMs), which are fully connected recurrent networks but without any connections between neurons of the same layer. Training is performed unsupervised using the standard CD rule~\citep{hinton2006fast} and only the output layer is trained in a supervised manner. The main difference between spiking DBNs and traditional DBNs is the activation function used for the neurons.~\cite{o2013real} proposed the use of the Siegert approximation~\citep{Jug_etal_2012} as the activation function, which returns the expected firing rate of a LIF neuron \DIFaddbegin \DIFadd{(Equation~\ref{eq:LIF}) }\DIFaddend given input firing rates, input weights, and standard neuron parameters. \DIFaddbegin \DIFadd{Further details regarding the training process can be found in \mbox{%DIFAUXCMD
\citet{o2013real}}.}\DIFaddend 
...
The pixels of each MNIST digit from the testing set are converted into Poisson spike trains \DIFdelbegin \DIFdel{with a rate proportional to the intensity of their pixel, while their firing rates are scaled so that the total firing rate of the input population is constant~\mbox{%DIFAUXCMD
\citep{o2013real}}.}
\DIFdelend \DIFaddbegin \DIFadd{as described in Section~\ref{sec:poissonian}.
}\DIFaddend The CA was chosen as the performance metric of the spiking DBN, which is the percentage of the correctly classified digits over the whole MNIST testing set.
...
\DIFaddendFL \caption{Power dissipation of a spiking DBN running on a single SpiNNaker chip as a function of the total \DIFdelbeginFL \DIFdelFL{number of }\DIFdelendFL input \DIFdelbeginFL \DIFdelFL{spikes per second}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{firing rate}\DIFaddendFL .}
	\label{Fig:spinnchipPower}
\end{figure} 

\section{Discussion}
\label{sec:summ}
\subsection{\textcolor{blue}{Summary of the work}}
...
Since new problems will \DIFdelbegin \DIFdel{be introduced continuously }\DIFdelend \DIFaddbegin \DIFadd{continuously arise }\DIFaddend before vision becomes a solved question, the dataset will \DIFdelbegin \DIFdel{evolve }\DIFdelend \DIFaddbegin \DIFadd{keep evolving }\DIFaddend as research develops. 
...
The first \DIFdelbegin \DIFdel{launch }\DIFdelend \DIFaddbegin \DIFadd{version }\DIFaddend of the dataset is published as NE15-MNIST, which contains four different spike presentations of the stationary hand-written digit database.
...
The complementary evaluation methodology is essential to assess both the model-level and hardware-level performances \DIFdelbegin \DIFdel{.
	...
	While the recognition accuracy, network latency and also the biological time taken for both training and testing are specific performance measurements of a spike-based model}\DIFdelend \DIFaddbegin \DIFadd{for SNNs}\DIFaddend .
\DIFaddbegin \DIFadd{Except for classification accuracy, the response latency and number of events are the other specific evaluation metrics for spike-based processing.
	Moreover, to describe a SNN model in a sufficient way is also important to share a network design and the particular SNN characteristics are highlighted in the paper. }\DIFaddend To build any SNN model on a hardware platform, its network size will be constrained by the scalability of the hardware.
...
\DIFdelbegin \DIFdel{The accuracy of the results (e.g. CA) are naturally affected by the precision of the variable representing the membrane potential and synaptic weights.
}\DIFdelend Any attempt to implement an on-line learning algorithm on neuromorphic hardware must be backed by synaptic plasticity support.
\DIFdelbegin \DIFdel{Running }\DIFdelend \DIFaddbegin \DIFadd{Therefore running }\DIFaddend an identical SNN model on different neuromorphic hardware \DIFdelbegin \DIFdel{platforms can not only expose if any of }\DIFdelend \DIFaddbegin \DIFadd{exposes the feasibility of such platforms.
	If the model runs smoothly on a hardware platform, it then can benchmark }\DIFaddend the \DIFdelbegin \DIFdel{previously mentioned capacities are supported, but also benchmark their performance on simulation time and energy usage.}
\DIFdel{Using the Poissonian subset of the NE15-MNIST dataset, two benchmark systems were proposed. 
	The models were described and their performance on accuracy, network latency, }\DIFdelend \DIFaddbegin \DIFadd{performance of the hardware simulator on }\DIFaddend simulation time and energy usage\DIFdelbegin \DIFdel{were presented.
	These example benchmarking systems provided a recommended way of using the dataset and evaluating system performance.
	They provide a baseline for comparisons and encourage improved algorithms and models to make use of the dataset}\DIFdelend .
\DIFaddbegin \DIFadd{CA is also a metric on hardware evaluation because of the precision limit of the membrane potential and synaptic weights.}\DIFaddend 
...
As \DIFdelbegin \DIFdel{the dataset }\DIFdelend \DIFaddbegin \DIFadd{far as we know, it is the first attempt benchmarking neuromorphic vision recognition by providing public spike-based dataset and the evaluation metrics.
...
As the dataset }\DIFaddend grows, it will allow new problems to be investigated by researchers, which should allow the identification of future directions and, in consequence, advance the field.

\subsection{The future direction of an evolving database}
The database will \DIFdelbegin \DIFdel{expand }\DIFdelend \DIFaddbegin \DIFadd{be expanded }\DIFaddend by converting more popular vision datasets to spike representations.
...
\DIFaddbegin \DIFadd{There will be more methods and algorithms of converting images to spikes.
	Although Poisson spikes are the most commonly used external input of an SNN system, there are several }\textit{\DIFadd{in-vivo}} \DIFadd{recordings in different cortical areas showing that the inter-spike intervals (ISI) are not Poissonian~\mbox{%DIFAUXCMD
		\citep{deger2012statistical}}%DIFAUXCMD
	. 
	Thus~\mbox{%DIFAUXCMD
		\citet{deger2012statistical} }%DIFAUXCMD
	proposed new algorithms to generate superposition spike trains of Poisson process with dead-time (PPD) and of Gamma process.
	Including novel spike generation algorithms to the dataset is one of the future work to satisfy the large varieties of research.}\DIFaddend
...
By doing this, huge number of videos, such as in YouTube, can automatically be converted to spikes, therefore providing researchers more time to work on their own applications\DIFaddbegin \DIFadd{.}

\DIFadd{In all, it is impossible to provide enough datasets, converting methods or benchmarking results by the dataset proposers ourselves, thus we encourage researchers contribute to the dataset.
	...
	and neuromorphic hardware owners are welcome to provide benchmarking results to compare the system performance}\DIFaddend .
