...
%DIF DEL NEbenchmark_steve.tex   Sun Feb 14 12:41:32 2016
%DIF ADD NEbenchmark.tex         Sun Feb 14 17:46:49 2016
...
Finally, we demonstrate the use of the dataset and the evaluation methodology using two SNN models to validate \DIFdelbegin \DIFdel{their classification capabilities and }\DIFdelend the performance of \DIFaddbegin \DIFadd{the models and }\DIFaddend their hardware implementations.
...
An \DIFdelbegin \DIFdel{event driven }\DIFdelend \DIFaddbegin \DIFadd{event-driven }\DIFaddend Contrastive Divergence (CD) training algorithm for Restricted Boltzmann Machines (RBMs) was proposed for Deep Belief Networks (DBNs) using LIF neurons with Spike-Timing-Dependent Plasticity (STDP) synapses and verified on MNIST with a CA of 91.9\%~\citep{neftci2013event}.
...
A computer simulation achieved \DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend 93.3\% CA on MNIST and had the potential to be implemented using memristors~\citep{bill2014compound}. 
...
The recorded AER data \DIFdelbegin \DIFdel{contains }\DIFdelend consists of a list of events, each one containing the time stamp of a spike and the address of the neuron which generated the spike.
...
\begin{itemize}
	...
	to encourage research into fast recognition methods \DIFdelbegin \DIFdel{based on, for example, the primate visual pathway}\DIFdelend \DIFaddbegin \DIFadd{to mimic the rapid and accurate ``core recognition'' in the primate ventral visual pathway~\mbox{%DIFAUXCMD
\citep{dicarlo2012does}
}%DIFAUXCMD
}\DIFaddend .
...
Python code is \DIFdelbegin \DIFdel{provide }\DIFdelend \DIFaddbegin \DIFadd{provided }\DIFaddend for converting from either file format to the other.
...
A different way of encoding spikes is \DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{to use }\DIFaddend a rank-order code; this means
...
Since DoGs are used as \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend means to encode the image, and they do not form an orthogonal set of basis functions, the algorithm also performs a redundancy correction step.
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}
		...
		\If{$Q(m, c) \neq 0$} \DIFdelbegin %DIFDELCMD < \Comment{Adjust only near}
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \Comment{Adjust only spatially near coefficients}
		\DIFaddend \State $c \leftarrow c - m \times Q(m, c)$
		\EndIf
		\EndFor
		\Until{$C = \emptyset$}
...
\label{subsec_flash}
The purpose of including the subset with DVS-recorded flashing digits is to promote the \DIFdelbegin \DIFdel{application of Rank-Order-Coding to DVS output, and to accelerate fast on-set recognition by using just the early 30}%DIFDELCMD < \% %%%
\DIFdel{part of the spike train}\DIFdelend \DIFaddbegin \DIFadd{research on rapid and accurate ``core recognition'', thus to encourage applying non-rate-based algorithms, for example ROC, to short DVS output spike trains}\DIFaddend .
...
%	}
	...
	\DIFaddbeginFL \DIFaddFL{Blue is used for `ON' events and green for `OFF' events.
	(a) The raster plot.
	(b) }\DIFaddendFL The bursty nature of the resulting spikes is illustrated, where most of the spikes occur in a 30~ms time slot.\DIFdelbeginFL \DIFdelFL{Blue is used for `ON' events and green for `OFF' events.}\DIFdelendFL }
	\label{fig:flash}
\end{figure}
...
A total of \DIFdelbegin \DIFdel{30,000 }\DIFdelend \DIFaddbegin \DIFadd{$30,000$ }\DIFaddend recordings were made: 10 digits, at 3 different scales, \DIFdelbegin \DIFdel{1,000 }\DIFdelend \DIFaddbegin \DIFadd{$1,000$ }\DIFaddend different handwritten samples for each.
...
The number of synaptic events can be measured as \DIFdelbegin \DIFdel{`Sopbs'}\DIFdelend \DIFaddbegin \DIFadd{``Sopbs''}\DIFaddend , synaptic operations per biological second.
...
The main characteristics include the topology \DIFdelbegin \DIFdel{, }\DIFdelend and the neural and synaptic models.
The network topology defines the number of neurons used for each layer \DIFdelbegin \DIFdel{, }\DIFdelend and the connections between layers and neurons.
...
Any non-neural classifier, sometimes added to aid the design, sometimes to enhance the output of the network, must also be specified.
...
This should include details of the way samples were presented: spiking rates, and biological time per \DIFdelbegin \DIFdel{testing }\DIFdelend \DIFaddbegin \DIFadd{test }\DIFaddend sample.
...
The first case study is a simple two-layer network where the input neurons receive \DIFdelbegin \DIFdel{Poissonian }\DIFdelend \DIFaddbegin \DIFadd{Poisson }\DIFaddend spike trains from the dataset and form a fully connected network with the decision neurons.
...
Every \DIFdelbegin \DIFdel{testing }\DIFdelend \DIFaddbegin \DIFadd{test }\DIFaddend image ($10,000$ images in total) is presented once and lasts 1~s with a silence of 200~ms between consecutive images.
...
\DIFaddbegin \DIFadd{The CA drops less than $0.06\%$ on SpiNNaker compared to NEST due to the limited fast memory and the necessity for fixed-point arithmetic on SpiNNaker to ensure real-time operation.
...
The spike rates of the \DIFdelbegin \DIFdel{testing }\DIFdelend \DIFaddbegin \DIFadd{test }\DIFaddend samples were also modified, ranging from 10 to $5,000$~Hz.
...
The variations in the total Sopbs \DIFdelbegin \DIFdel{ly }\DIFdelend \DIFaddbegin \DIFadd{lie }\DIFaddend in the firing rate of the output layers only, and the stronger connections lead to the higher firing rates.
...
\DIFdelbegin \DIFdel{Thus the network size varies according to the number of neurons in the decision layer where each neuron represents a subclass of a digit and the template is embedded in the synaptic weights connecting from the input layer to the decision neuron.
...
.
}\DIFdelend %DIF > Thus the network size varies according to the number of neurons in the decision layer where each neuron represents a subclass of a digit and the template is embedded in the synaptic weights connecting from the input layer to the decision neuron.
\DIFaddbegin 

\DIFaddend As the network size grows there are more decision neurons and synapses connecting to them, thus the simulation time on NEST increases.
...
\DIFdelbegin \DIFdel{The Thermal Design Power (TDP) usage of all four processors of i5-4570 actively operating at base frequency is 84~W}\footnote{%DIFDELCMD < \url{http://ark.intel.com/products/75043/Intel-Core-i5-4570-Processor-6M-Cache-up-to-3_60-GHz}%%%
}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.
}\DIFdelend %DIF > The Thermal Design Power (TDP) usage of all four processors of i5-4570 actively operating at base frequency is 84~W\footnote{\url{http://ark.intel.com/products/75043/Intel-Core-i5-4570-Processor-6M-Cache-up-to-3_60-GHz}}.
...
\begin{table}[h]
    ...
    & &\multicolumn{5}{c}{\vspace*{-4mm}}\\
		...
		& 767.67
		& $1,131.09$
		& \DIFdelbeginFL \DIFdelFL{$120,27.75$
		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$12,027.75$
		}\DIFaddendFL \\
		time (s)
		& S
...
\DIFdelbegin \DIFdel{As the dataset grows, it will allow new problems to be investigated by researchers, which should allow the identification of future directions and, in consequence, advance the field.
}\DIFdelend %DIF > As the dataset grows, it will allow new problems to be investigated by researchers, which should allow the identification of future directions and, in consequence, advance the field.
