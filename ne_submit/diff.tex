...
\DIFaddbegin \DIFadd{In terms of vision recognition on neuromorphic hardware, TrueNorth~\mbox{%DIFAUXCMD
\citep{merolla2014million}
}%DIFAUXCMD
has been used to detect and recognise five classes of object in the video clips of the Neovision2 Tower dataset.
\mbox{%DIFAUXCMD
\cite{Qiao2015re}
}%DIFAUXCMD
presented an analogue on-line learning neuromorphic classification system which responds selectively to images of cars and motorbikes selected from the Caltech 101 dataset.
Generic training algorithms for pattern recognition
~\mbox{%DIFAUXCMD
\citep{schmuker2014neuromorphic}
}%DIFAUXCMD
and regression/classification tasks
~\mbox{%DIFAUXCMD
\citep{thakur2016low}
}%DIFAUXCMD
have been implemented on neuromorphic platforms. }\DIFaddend 
...
\end{itemize}
The dataset can be found in the GitHub repository at:
\DIFdelbegin \DIFdel{https://github.com/qian-liu/benchmarking }\DIFdelend
\DIFaddbegin \DIFadd{https://github.com/NEvision/NE15. }\DIFaddend 
...
%	}
	...
	(a) The raster plot\DIFaddbegin \DIFadd{ shows spikes generated by individual neurons over time.
	It is hard to recognise the total number of spikes due to the large number of neurons involved in the figure.
	Thus all the spikes are ordered in time, and displayed in the figure below}\DIFaddend.
	(b) \DIFdelbegin \DIFdel{The bursty nature of the resulting spikes is illustrated, where most of the spikes occur in a 30~ms time slot.}\DIFdelend
	\DIFaddbegin \DIFadd{The raster plot shows ordered spike sequence over time.
	The total number of spikes are around $7,000$ for both `ON' and `OFF' events.
	The bursty nature of the resulting spikes is illustrated, where most of the spikes occur in a 30~ms time slot. }\DIFaddend}
	\label{fig:flash}
\end{figure}
...
\DIFaddbegin \DIFadd{A suggested description of an SNN model is shown in Table~\ref{tb:model_eval} where the performance evaluation metrics are in bold and the SNN specific description is in italics. }\DIFaddend
...
First of all, the input of an SNN \DIFdelbegin \DIFdel{should be}\DIFdelend \DIFaddbegin \DIFadd{model is }\DIFaddend specified.
...
Secondly, as with the evaluation of conventional artificial neural networks, a description of the network characteristics \DIFdelbegin \DIFdel{should be provided since it is}\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend the basis for the overall performance evaluation.
...
\DIFdelbegin \DIFdel{A clear distinction should be made between supervised, semi-supervised and unsupervised learning. }\DIFdelend 
\DIFaddbegin \DIFadd{Specifying the learning algorithm with its mechanism (supervised, semi-supervised and unsupervised) helps the reader undersdand the core features of the model. }\DIFaddend
...
This \DIFdelbegin \DIFdel{should }\DIFdelend includes details of the way samples were presented: spiking rates, and biological time per test sample.
...
\DIFaddbegin \DIFadd{Following to the formatted evaluation as in Table~\ref{tb:model_eval}, Table~\ref{tb:software_comparison} lists a few SNN models of MNIST classification, although some detailed description is missing. }\DIFaddend 
\DIFdelbegin \DIFdel{Most of these reports did not provide sufficiently detailed descriptions of the networks and the models were not completely evaluated in the way that we propose. }
...
\DIFaddbegin \DIFadd{While software modelling provides great flexibility, digital platforms generally have reduced precision (due to the inherent discretisation) and higher energy consumption when compared to analogue platforms. Furthermore, the processing cores used in SpiNNaker chips perform better when using integer or fixed-point arithmetic~\mbox{\citep{Hopkins2015Accuracy}}. 
And the requirement for the models to run in real time leads to constraints a the complexity of model that can be supported. }\DIFaddend
...
These two measures are equivalent, since J/SE$\times$Sops/W = 1.
 \DIFaddbegin \DIFadd{To investigate the power usage in detail, \mbox{\cite{Diamond2016comparing}} included the power drawn by the attached workstations.
This work indicated the power variation by each task over time. }\DIFaddend
...
The weak weights were set to inhibitory connections with identical strengths\DIFaddbegin \DIFadd{ in testing.
The negative connections reduced the output firing rates while keeping a good classification ability. }\DIFaddend
...
On the other hand, SpiNNaker works in real (biologically real) time and the simulation time becomes shorter than the NEST simulation when $1,000$ patterns per digit\DIFaddbegin \DIFadd{ ($1,000$ decision neurons per digit) }\DIFaddend are used.
...
\DIFaddbegin \DIFadd{A complete SNN simulation contains three main stages: pre-processing (such as network building and input data generation), simulation and post-processing (for example, extracting recorded data).
\mbox{\citet{Diamond2016comparing}} compared the power usage on three hardware platforms in all the stages while this paper focuses on the simulation part.
...
Among different network configurations, the model with 500 decision neurons (50 clusters per digit) reaches the highest recognition rate of 92.99\% on average having a latency of \DIFdelbegin \DIFdel{about }\DIFdelend 13.82~ms \DIFaddbegin \DIFadd{mean and 2.96~ms standard deviation.
And there are standard deviations of 2.57\% on CA and of 1.17~ms on the latency over 10 testing digits. }\DIFaddend
...
The NEST simulation costs 767.67~s on average for the entire $12,000$~s biological-time test, 20~W in power use on the PC and $15.35$~KJ of energy, while SpiNNaker works in real time using $4.92$~KJ of energy at a power of 0.41~W\DIFdelbegin \DIFdel{.
Providing the number of synaptic events (Sopbs) respectively for all the configurations and the simulation time, the energy per synaptic event on a hardware platform can be calculated }\DIFdelend (see Table~\ref{tbl:compare})\DIFdelbegin \DIFdel{: 
306~nJ/SE running NEST on a PC, and 98~nJ/SE on SpiNNaker}\DIFdelend.
...
\DIFdelbegin \DIFdel{For this work we used the identical }\DIFdelend \DIFaddbegin \DIFadd{This section aims to review and reinterpret  results from previously published studies~\mbox{\citep{Stromatias2015LiveDemo,Stromatias2015scalable,stromatias2015robustness}}, which utilised }\DIFaddend
off-line trained\footnote{\url{https://github.com/dannyneil/edbn/}} spiking DBN as presented by \citet{o2013real}, \DIFdelbegin \DIFdel{which}\DIFdelend \DIFaddbegin \DIFadd{This particular DBN }\DIFaddend comprises 784 neurons for the input layer, two hidden layers with 500 neurons each, and an output layer with 10 neurons. This is abbreviated as a 784-500-500-10 architecture. Simulations take place on a software spiking neural network simulator [Brian~\citep{goodman2008brian}] and results are verified on the SpiNNaker platform.
...
\begin{figure}[hbt!]
	...
	\citep{stromatias2015robustness}.}
...
Finally, this particular spiking DBN ran on a single SpiNNaker chip (16 ARM9 cores) and dissipated \DIFdelbegin \DIFdel{less than}\DIFdelend \DIFaddbegin \DIFadd{about }\DIFaddend 0.3~W when $1,500$ spikes per second per digit were used\DIFdelbegin \DIFdel{, $1,441,723$~Sopbs were generated and 214~nJ/SE of energy was dissipated}\DIFdelend \DIFaddbegin \DIFadd{.
The number of synaptic events generated was $1.88$M~Sopbs and less than 2.97~KJ of energy was consumed running the whole testing set over $10,000$~s, }\DIFaddend as seen in Figure~\ref{Fig:spinnchipPower}. The identical network executed on Minitaur~\citep{neil2014minitaur}, an event-driven FPGA implementation, dissipated 1.5~W when $1,000$ spikes per image were used, and achieved a CA of 92.0\%.
\begin{figure}[hbt!]
	\centering
	\includegraphics[width=0.48\textwidth]{fig12_m}
	\caption{\DIFdelbegin \DIFdel{Power dissipation and energy per synaptic event}\DIFdelend \DIFaddbegin \DIFadd{Total energy consumption and number of SE per second }\DIFaddend of a spiking DBN running on a single SpiNNaker chip as a function of the total input firing rate.}
...
\DIFaddbegin \DIFadd{\mbox{\cite{orchard2015convert}} have presented a neuromorphic dataset using a similar approach, but the spike trains were obtained with micro-saccades.
...
Therefore, the datasets complement each other.}\DIFaddend
...
\DIFaddbegin \DIFadd{In accordance with the suggestions from~\mbox{\cite{tan2015bench}}, the evaluation metrics highlight the strengths of spike-based vision tasks and the dataset design also promotes the research into rapid and low energy recognition (e.g. flashing digits). }\DIFaddend
