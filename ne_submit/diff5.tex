...
%DIF DEL NEbenchmark_revise3.tex   Thu Jul 28 17:41:07 2016
%DIF ADD NEbenchmark.tex           Thu Jul 28 16:55:48 2016
...
\def\Authors{Qian~Liu\,$^{1,*}$, Garibaldi~Pineda-Garc\'ia\,$^{1}$, Evangelos~Stromatias\,$^{2}$, Teresa~Serrano-Gotarredona\,$^{2}$, and Steve~Furber\,$^{1}$}
...
%DIF 98c98
%DIF < \def\corrAddress{SpiNNaker, Advanced Processor Technologies Research Group, School of Computer Science, The University of Manchester, Oxford Road, Manchester, M13 9PL, United Kingdom}
%DIF -------
\def\corrAddress{SpiNNaker, Advanced Processor Technologies Research Group, School of Computer Science, The University of Manchester, Oxford Road, Manchester, M13~9PL, United Kingdom} %DIF > 
%DIF -------
...
Today\DIFaddbegin \DIFadd{, }\DIFaddend increasing attention is being paid to research into spike-based neural computation \DIFdelbegin \DIFdel{in order }\DIFdelend both to gain a better understanding of the brain and \DIFdelbegin \DIFdel{also }\DIFdelend to explore biologically-inspired computation.
Within this field, the primate visual pathway and its hierarchical organisation have been extensively studied \DIFdelbegin \DIFdel{, }\DIFdelend and Spiking Neural Networks (SNNs)\DIFaddbegin \DIFadd{, }\DIFaddend inspired by the understanding of observed biological structure and function\DIFaddbegin \DIFadd{, }\DIFaddend have been successfully applied to visual recognition and classification tasks.
...
In this paper we first propose an initial NE (Neuromorphic Engineering) dataset \DIFdelbegin \DIFdel{that is }\DIFdelend based on standard computer vision benchmarks and uses digits from the MNIST database. This dataset is compatible with the state of current research on spike-based image recognition.
...
Achieving the brain's energy efficiency motivates the development of neuromorphic hardware, since the \DIFdelbegin \DIFdel{brain only consumes }\DIFdelend \DIFaddbegin \DIFadd{human brain consumes only }\DIFaddend about 20~W of power~\citep{drubach2000brain}.
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. \DIFaddbegin \DIFadd{in }\DIFaddend 200~ms in monkeys~\citep{fabre1998rapid}\DIFdelbegin \DIFdel{and the images are presented sequentially in spikes }\DIFdelend \DIFaddbegin \DIFadd{, even with short appearance (}\DIFaddend less than 100~ms\DIFaddbegin \DIFadd{) of the target objects}\DIFaddend ~\citep{keysers2001speed}.
Such rapid and highly accurate recognition \DIFdelbegin \DIFdel{are the targets }\DIFdelend \DIFaddbegin \DIFadd{is the target }\DIFaddend of modelling spike-based visual recognition.
...
Inspired by biological studies \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend the visual ventral pathway, SNN models have successfully been adapted to \DIFdelbegin \DIFdel{computer vision }\DIFdelend \DIFaddbegin \DIFadd{visual }\DIFaddend recognition. \cite{riesenhuber1999hierarchical} proposed a quantitative modelling framework for object recognition with position-, scale- and view-invariance.
...
It was capable of recognising three moving postures with a CA of about 99.48\% and \DIFaddbegin \DIFadd{classifying hand-written digits with }\DIFaddend 88.14\% \DIFaddbegin \DIFadd{accuracy }\DIFaddend on the MNIST-DVS dataset (see \DIFdelbegin \DIFdel{Chapter}\DIFdelend \DIFaddbegin \DIFadd{Section}\DIFaddend ~\ref{sec:data}).
...
Spike-Timing-Dependent Plasticity (STDP) as  a \DIFdelbegin \DIFdel{biological learning process }\DIFdelend \DIFaddbegin \DIFadd{learning mechanism based on biological observations }\DIFaddend has been applied to vision tasks.
...
In consequence, a new set of spike-based vision datasets is now needed to quantitatively measure progress within \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend rapidly advancing field \DIFaddbegin \DIFadd{of spike-based visual recognition }\DIFaddend and to provide resources to support fair competition between researchers.

Apart from using spikes instead of the frame-based data used in conventional computer vision, new concerns \DIFdelbegin \DIFdel{on the latency and energy cost }\DIFdelend arise when evaluating neuromorphic vision\DIFaddbegin \DIFadd{, such as latency and energy consumption, }\DIFaddend in addition to recognition accuracy.
...
%	Other spike representation schemes may do a better job of preserving information from the original stimuli.
	...
	\DIFdelbegin \DIFdel{In order to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend reveal the advantages of neuromorphic engineering: \DIFdelbegin \DIFdel{high-accurate}\DIFdelend \DIFaddbegin \DIFadd{highly 	accurate}\DIFaddend , rapid, energy-efficient recognition, we need not only a spike-based dataset but also an appropriate evaluation methodology.
...
NE15-MNIST is the spiking version of an original non-spiking dataset which was downloaded from the MNIST Database of Handwritten Digits~\citep{lecun1998gradient}  website\footnote{http://yann.lecun.com/exdb/mnist/}.
...
\DIFdelbegin \DIFdel{While in neuromorphic vision, MNIST is still an unsolved problem but }\DIFdelend \DIFaddbegin \DIFadd{MNIST is }\DIFaddend a popular task among the \DIFaddbegin \DIFadd{neuromorphic vision }\DIFaddend research community as stated in Section~\ref{sec:intro}.
...
\subsubsection{\DIFdelbegin \DIFdel{Rank-Order-Encoding}\DIFdelend \DIFaddbegin \DIFadd{Rank Order Encoding}\DIFaddend }
A different way of encoding spikes is to use a \DIFdelbegin \DIFdel{rank-order }\DIFdelend \DIFaddbegin \DIFadd{rank order }\DIFaddend code; this means
...
\DIFdelbegin \DIFdel{Rank-ordered coding }\DIFdelend \DIFaddbegin \DIFadd{Rank order coding (ROC) }\DIFaddend can be performed using an algorithm known as the
{FoCal algorithm~\citep{sen2009evaluating}}.
This algorithm models the foveal pit region, the highest resolution area of the retina, with four ganglion cell layers each with a different scale of centre-surround receptive field~\citep{kolb2003retina}. \DIFdelbegin \DIFdel{In order to }\DIFdelend \DIFaddbegin \DIFadd{To }\DIFaddend simulate these layers two steps are required: the first consists of four discrete 2D convolutions; the second removes redundant information produced in the first step. During the first step, the centre-surround behaviour of the ganglion cells is modelled using Difference of Gaussians~(DoG) kernel for convolution. 
\begin{equation}
\label{eq-dog}
...
\label{sec:eval}
As a result of the spike-based processing used in SNN models, new concerns on the latency and energy cost arise over performance assessment.
...
This model-specific assessment provides more robust comparisons between hardware platforms \DIFdelbegin \DIFdel{thanks to }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend using the same network topology, neuron and synaptic models, and learning rules. 
...
\begin{table*}[hbt!]
	...
	\begin{center}
		...
		\begin{tabular}{ l c c c c }
			...
			\begin{mycell}{3.5cm} Event-driven contrastive divergence (eCD), unsupervised \end{mycell}&  % training
			\begin{mycell}{3.5cm} 91.9\% \\ 1~s per test\end{mycell} \\% recognition

			\begin{mycell}{2.5cm}~\cite{neftci2016stochastic} \end{mycell} & 
			\begin{mycell}{1.9cm} Thresholding,\\ Synaptic current\end{mycell} & % preprocessing
			\begin{mycell}{3.5cm} Two layer RBM, \\ LIF neurons \end{mycell}&  % network
			\begin{mycell}{3.5cm} Synaptic Sampling Machine + eCD, unsupervised \end{mycell}&  % training
			\begin{mycell}{3.5cm} 95.8\% \\ 250~ms per test\end{mycell} \\% recognition
			...
			\begin{mycell}{3.5cm} Unsupervised, STDP,\\ %adaptive membrane potential, 
%				$3,000,000$~s of training\\ 
			...
			\begin{mycell}{3.5cm} 91.3\%(Thresholding)\\ 11~s per test \\ 88.1\%(DVS),\\ 2~s per test\end{mycell}\\ % recognition
      ...
      \begin{minipage}{\textwidth}
        ...
        \begin{savenotes}
  		...
  		\begin{tabular}{l c c c c c c}
  			...
  			\begin{mycell}{2.0cm}11- to 14-bit \DIFdelbeginFL \DIFdelFL{synapses}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapse}\DIFaddendFL \end{mycell} & 
          ...
          \footnote[1]{We consider them 4-bit \DIFdelbeginFL \DIFdelFL{synapses }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapse }\DIFaddendFL because it is only possible to choose between 4 different signed integers and whether the synapse is active or not.}
  			\end{mycell}& 
  			...
  			\begin{mycell}{2.0cm}13-bit shared \\ \DIFdelbeginFL \DIFdelFL{synapses}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapse}\DIFaddendFL \end{mycell} &
  			\begin{mycell}{2.0cm}Real-time\end{mycell} &
  			...
  			\begin{mycell}{2.0cm}Fixed models,\\Config params\end{mycell}& 
  			\begin{mycell}{2.0cm}Fixed rule\end{mycell}& 
  			\begin{mycell}{2.0cm}4-bit/\\16 values\\\DIFdelbeginFL \DIFdelFL{synapses}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{synapse}\DIFaddendFL \end{mycell}& 
...
And the requirement for the models to run in real time leads to constraints \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend the complexity of model that can be supported.
...
\DIFdelbegin \DIFdel{To investigate the power usage in detail, \mbox{%DIFAUXCMD
\cite{Diamond2016comparing}
}%DIFAUXCMD
included the power drawn by the attached workstations.
This work indicated the power variation by each task over time.
}\DIFdelend %DIF > To investigate the power usage in detail, \cite{Diamond2016comparing} included the power drawn by the attached workstations.
%DIF > This work indicated the power variation by each task over time.
...
In terms of this case study, the performance of the model was evaluated with both software simulation \DIFdelbegin %DIFDELCMD < [%%%
\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend on NEST~\citep{gewaltig2007nest}\DIFdelbegin %DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend and hardware implementation (on SpiNNaker).
...
\DIFdelbegin \DIFdel{Every test image (}\DIFdelend \DIFaddbegin \DIFadd{One test trial contains }\DIFaddend $10,000$ images in total \DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{and each image }\DIFaddend is presented once and lasts 1~s with a silence of 200~ms between consecutive images.
...
\DIFdelbegin \DIFdel{A complete SNN simulation contains three main stages: pre-processing (such as network building and input data generation), simulation and post-processing (for example, extracting recorded data).
\mbox{%DIFAUXCMD
\citet{Diamond2016comparing}
}%DIFAUXCMD
compared the power usage and simulation time on three hardware platforms in detail regarding to all the stages while this paper focuses on the simulation part only.
}\DIFdelend %DIF > A complete SNN simulation contains three main stages: pre-processing (such as network building and input data generation), simulation and post-processing (for example, extracting recorded data).
%DIF > \citet{Diamond2016comparing} compared the power usage and simulation time on three hardware platforms in detail regarding to all the stages while this paper focuses on the simulation part only.
...
%\citet{Diamond2016comparing} compared the power usage on three hardware platform in detail of all the stages while this paper focuses on the simulation part.
...
Simulations take place on a software spiking neural network simulator\DIFdelbegin %DIFDELCMD < [%%%
\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend Brian~\citep{goodman2008brian}\DIFdelbegin %DIFDELCMD < ] %%%
\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend and results are verified on the SpiNNaker platform.
...
%Later,~\cite{neil2014minitaur} used the identical offline trained network to map it on an event-driven FPGA implementation called Minitaur.
...
The \DIFdelbegin \DIFdel{rank-order-encoded }\DIFdelend \DIFaddbegin \DIFadd{rank-order coded }\DIFaddend subset, FoCal, encourages research into spatio-temporal algorithms on recognition applications using only small numbers of input spikes.
...
This dataset aims to convert static images to neuromorphic vision input, while the recordings of moving input in \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend paper are intended to promote position-invariant recognition.
