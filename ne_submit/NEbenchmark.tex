...
Within this field, the primate visual pathway and its hierarchical organisation have been extensively studied and Spiking Neural Networks (SNNs), inspired by the understanding of observed biological structure and function, have been successfully applied to visual recognition and classification tasks.
...
In this paper we first propose an initial NE (Neuromorphic Engineering) dataset based on standard computer vision benchmarks and uses digits from the MNIST database. This dataset is compatible with the state of current research on spike-based image recognition.
...
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. in 200~ms in monkeys~\citep{fabre1998rapid}, even with short appearance (less than 100~ms) of the target objects~\citep{keysers2001speed}.
...
%	Other spike representation schemes may do a better job of preserving information from the original stimuli.
	...
	To reveal the advantages of neuromorphic engineering: highly 	accurate, rapid, energy-efficient recognition, we need not only a spike-based dataset but also an appropriate evaluation methodology.
      ...
      \begin{minipage}{\textwidth}
        ...
        \begin{savenotes}
  		...
  		\begin{tabular}{l c c c c c c}
  			...
  			\begin{mycell}{2.0cm}11- to 14-bit synapse\end{mycell} & 
          ...
          \footnote[1]{We consider them 4-bit synapse because it is only possible to choose between 4 different signed integers and whether the synapse is active or not.}
  			\end{mycell}& 
  			...
  			\begin{mycell}{2.0cm}13-bit shared \\ synapse\end{mycell} &
  			\begin{mycell}{2.0cm}Real-time\end{mycell} &
  			...
  			\begin{mycell}{2.0cm}Fixed models,\\Config params\end{mycell}& 
  			\begin{mycell}{2.0cm}Fixed rule\end{mycell}& 
  			\begin{mycell}{2.0cm}4-bit/\\16 values\\synapse\end{mycell}& 
...
And the requirement for the models to run in real time leads to constraints on the complexity of model that can be supported.
