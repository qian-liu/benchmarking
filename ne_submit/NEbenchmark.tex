...
Achieving the brain's energy efficiency motivates the development of neuromorphic hardware, since the brain only consumes about 20~W of power~\citep{drubach2000brain}.
In the case of visual processing, the brain can accurately recognise objects remarkably quickly, e.g. 200~ms in monkeys~\citep{fabre1998rapid} and the images are presented sequentially in spikes less than 100~ms~\citep{keysers2001speed}.
...
Apart from using spikes instead of the frame-based data used in conventional computer vision, new concerns on the latency and energy cost arise when evaluating neuromorphic vision in addition to recognition accuracy.
...
%	Other spike representation schemes may do a better job of preserving information from the original stimuli.
	...
	In order to reveal the advantages of neuromorphic engineering: high-accurate, rapid, energy-efficient recognition, we need not only a spike-based dataset but also an appropriate evaluation methodology.
