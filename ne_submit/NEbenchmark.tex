...
Section~\ref{sec:method} elaborates on the purpose and protocols of the proposed dataset, describes the sub-datasets and their generation methods, it also demonstrates the suggested evaluation methodology in accordance with the dataset.
...
The same digit displayed in Figure~\subref*{Fig:jaer} is converted to Poissonian spike trains, see Figure~\subref*{Fig:poisson}.
The raster plot can be found in Figure~\subref*{Fig:raster}, indicating the intensities of the pixels.
...
Figure~\ref{fig-reconstruction-results} shows the reconstruction results for the two stages of the algorithm. On Figure~\subref*{pic-lena-reconstructed-raw} the reconstruction was applied after the convolution but without the FoCal correction, a blurry image is the result of redundancy in the spike representation. A better reconstruction can be obtained after Algorithm~\ref{code-focal-corr} has been applied, the result is shown in Figure~\subref*{pic-lena-reconstructed-focal}.
...
Figure~\subref*{Fig:train} shows how the connections to a single decision neuron are tuned.
...
The trained weights are plotted in accordance with the positions of the decision neurons in Figure~\subref*{Fig:test}.
\begin{figure}[thb!]
	\centering
...
The feed-forward testing network is shown in Figure~\subref*{Fig:test} where Poissonion spike trains are generated the same way as in the training with a total firing rate of $2,000$~Hz per image.
...
Figure~\subref*{fig:acc_time} shows that the CA is better as exposure time increases. The longer an image is presented, the more information is gathered by the network, so the accuracy climbs.
Classification accuracy also increases when input spiking rates are augmented (Figure~\subref*{fig:acc_rate}).
...
Figure~\subref*{fig:lat_rate} indicates that the latency is shortened with increasing input rates with both the original and scaled-up weights.
When the spiking rate is less than $2,000$~Hz, the network with stronger weights has a much shorter latency.
As long as there are enough spikes to trigger the decision neurons to spike, increasing the test time will not make the network respond sooner~(Figure~\subref*{fig:lat_time}).
\begin{figure}[htb!]
	\centering
