...
\begin{table*}[hbt!]
	...
	\begin{center}
		...
		\begin{tabular}{ l c c c c }
			...
			\begin{mycell}{3.5cm} 92.99\%\\1~s per test\\13.82~ms latency\\$5.01$M Sops\end{mycell}\\ % recognition
		\end{tabular}
		\egroup
	\end{center}
...
Because of the time dimension and the spike-based processing off SNN, there are more performance evaluation metrics involved except for classification accuracy: recognition latency and event number.
...
The event number can be measured as Sops, synaptic operations per second.
...
\begin{table*}[hbt!]
	...
	\begin{center}
		...
		\begin{tabular}{ l l l l }
			...
			\begin{leftcell}{4.5cm} - \textbf{classification accuracy}\\ - \textbf{\textit{response latency}}\\ - \textbf{\textit{event number}} \\ - \textit{biological testing time}\\ - \textit{input spiking rate}  \end{leftcell}% recognition
		\end{tabular}
		\egroup
	\end{center}
	\label{tb:model_eval}
...
The recognition result is evaluated on all the metrics proposed: classification accuracy, response latency and event number.
...
The CA drops less than $0.05\%$ on SpiNNaker comparing to NEST due to the limited fast memory and the necessity for fixed-point arithmetic on SpiNNaker to ensure real-time operation.
...
The total event number is around $5.01$M Sops in biological time, where only 0.01M spikes generated by the output layer. 
...
Given the event number, the energy per synaptic event can be calculated (see Table~\ref{tbl:compare}): 
...
\begin{table}[h]
	...
	\begin{center}
		\begin{tabular} {r|c|c|c|c|c|c|c|c}
			...
			& N & S & N & S & N & S & N & S\\
			\hline
			1 & 79.63 & 79.57 & 445.09 & \multirow{5}{*}{$12,000$} &20 & 0.38 & $7,386$  & $3,784$\\
			10 & 91.42 & 91.39 & 503.21 &   & 20  & 0.38 & 837 & 379 \\
			50 & 92.99 & 92.99 & $767.67$ &   & 20  & 0.41 & 255 & 82\\
			100 & 87.05 & 87.00 & $1,131.09$ &   &  19 & 0.44 & 179 & 44\\
			1000 & 89.63 & 89.58 & $120,27.75$ &   &  17  & 1.50 & 170 & 15\\
...
Except for classification accuracy, the response latency and event number are the other specific evaluation metrics for spike-based processing.
